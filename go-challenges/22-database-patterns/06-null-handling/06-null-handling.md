...
SQL NULL is not the same as a Go zero value; model it explicitly or lose information.
...
```bash
mkdir -p ~/go-exercises/06-null-handling.md
cd ~/go-exercises/06-null-handling.md
go mod init example.com/06-null-handling.md
go mod edit -go=1.26
```
...
func NewProfile(email string) (Profile, error) {
	if email == "" {
		return Profile{}, fmt.Errorf("profile: %w", ErrEmptyEmail)
	}
	return Profile{email: email}, nil
}

func (v Profile) Email() string {
	return v.email
}

func (v Profile) Ready() bool {
	return v.email != ""
}

func (v Profile) Describe() string {
	if !v.Ready() {
		return "not ready"
	}
	return fmt.Sprintf("profile:%s", v.email)
}
```
...
func TestNewProfile(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "valid", input: "primary", want: "profile:primary"},
		{name: "empty", input: "", err: ErrEmptyEmail},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()

			got, err := NewProfile(tt.input)
...
func ExampleNewProfile() {
	v, _ := NewProfile("primary")
	fmt.Println(v.Describe())
	// Output: profile:primary
}
```
...
import (
	...
	"example.com/06-null-handling"
)

func main() {
	v, err := nullable.NewProfile("primary")
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println(v.Describe())
...
Continue with [Migration Patterns](../07-migration-patterns/07-migration-patterns.md).
