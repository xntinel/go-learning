...
Offline database tests should verify repository contracts without depending on a networked server or external driver download.
...
```bash
mkdir -p ~/go-exercises/10-testing-with-in-memory-sqlite.md
cd ~/go-exercises/10-testing-with-in-memory-sqlite.md
go mod init example.com/10-testing-with-in-memory-sqlite.md
go mod edit -go=1.26
```
...
func NewFixture(name string) (Fixture, error) {
	if name == "" {
		return Fixture{}, fmt.Errorf("fixture: %w", ErrEmptyName)
	}
	return Fixture{name: name}, nil
}

func (v Fixture) Name() string {
	return v.name
}

func (v Fixture) Ready() bool {
	return v.name != ""
}

func (v Fixture) Describe() string {
	if !v.Ready() {
		return "not ready"
	}
	return fmt.Sprintf("fixture:%s", v.name)
}
```
...
func TestNewFixture(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "valid", input: "primary", want: "fixture:primary"},
...
import (
	...
	"example.com/10-testing-with-in-memory-sqlite"
)

func main() {
	v, err := memtest.NewFixture("primary")
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println(v.Describe())
...
This completes the database patterns chapter.
