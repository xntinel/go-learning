# Exercise 5: Sort, Limit, and Join Operators

With a value type, an operator interface, a catalog, and a filter in hand, this exercise adds the operators that combine and reshape streams: `SortOperator` (a blocking sort), `LimitOperator` (a streaming row gate), and the two general-purpose join algorithms, `NestedLoopJoinOperator` and `HashJoinOperator`. It also adds `Explain`, which renders an operator tree as indented text so a plan can be inspected rather than only run. The two correctness traps live here: NULL join keys must never match (SQL `NULL = NULL` is unknown, not true), and a LEFT join must surface every unmatched left row NULL-padded on the right.

This module is fully self-contained. It depends only on the standard library, ships its own demo and tests, and imports no other exercise: it carries its own copies of the value, operator, catalog, filter, and scan scaffolding so the join code has everything it needs.
...
sort-limit-join/
  ...
  join.go           SortOperator, LimitOperator, NestedLoopJoinOperator, HashJoinOperator, Explain
  cmd/
    demo/
      main.go        sort+limit, an inner hash join, and an Explain tree
  join_test.go       sort, limit, hash/nested-loop joins, NULL-key and LEFT-pad correctness
```

- Files: `value.go`, `operator.go`, `catalog.go`, `filter.go`, `scan.go`, `join.go`, `cmd/demo/main.go`, `join_test.go`.
- Implement: `SortOperator`/`NewSort`, `LimitOperator`/`NewLimit`, `JoinType`, `NestedLoopJoinOperator`/`NewNestedLoopJoin`, `HashJoinOperator`/`NewHashJoin`, and `Explain`; the value/operator/catalog/filter/scan files are reused scaffolding.
...
`SortOperator` is the lesson's first true pipeline-breaker. Its `Init` drains the entire child into a slice and runs `sort.SliceStable` under the ORDER BY keys before a single row leaves; the first `Next` cannot answer until the last input row has arrived, because that last row might sort to the front. The comparator walks the keys in order, falls through to the next key on a tie or an incomparable pair, and flips the sign for a descending key. Stability matters: a stable sort keeps equal-key rows in input order, which is what makes a multi-key sort composable from single-key passes and what the tests rely on.

`LimitOperator` is the opposite: a streaming gate that holds no buffer. It skips `offset` rows, emits up to `limit`, and then returns end-of-stream, so a `LIMIT 2` over a billion-row child stops the whole pipeline after two rows reach it. Pairing a blocking sort under a streaming limit is the naive `ORDER BY ... LIMIT k` plan; a later exercise replaces it with a bounded-heap Top-N that avoids sorting the whole input.

### Nested-loop join

`NestedLoopJoinOperator` is the most general join: for each outer row it rewinds and rescans the inner relation, evaluating an arbitrary condition, so it handles non-equi predicates that hashing cannot. The one rule that is easy to break is reinitialization: the inner operator must be `Init`-ed again for every new outer row, or it stays drained after the first pass and the join yields at most one block of rows. The operator also tracks whether the current outer row matched anything, so a LEFT join can emit that outer row NULL-padded on the right once its inner scan finishes with no match.

### Hash join, NULL keys, and outer padding

`HashJoinOperator` is an equi-join with a blocking build side. `Init` drains the build child into a hash table keyed by the join column; `Next` then streams the probe side, looking each probe key up in the table. The combined output tuple is always build followed by probe, which fixes the column order and the meaning of "left" and "right" for outer joins.

Two correctness rules are non-negotiable. First, NULL keys never match: a build row whose key is NULL is kept in the build-row list (so a LEFT join can still emit it NULL-padded) but is never inserted into the hash table, and a probe row whose key is NULL is never looked up. This is SQL three-valued logic — `NULL = NULL` is unknown, not true — and skipping the sentinel is the whole fix; hashing NULL to a fixed bucket would make NULL-keyed rows collide and join, producing rows that must not exist. Second, outer padding: a LEFT join, after the probe is drained, walks the build rows and emits every one that no probe row matched, NULL-padded on the right; a RIGHT join emits every unmatched probe row NULL-padded on the left. The build entry's `matched` flag is what distinguishes a build row that joined from one that did not.
...
The demo sorts users by age (NULL sorts first) then id and takes the first two, runs an inner hash join of users against orders, and prints the physical plan with `Explain`.
...
import (
	...
	planner "example.com/sort-limit-join"
)
...
func main() {
	...
	// Sort by age ASC (NULL first), then id ASC; take the first two.
	sortOp := planner.NewSort(
		planner.NewSeqScan(users, nil),
		[]planner.SortKey{{ColIndex: 2, Desc: false}, {ColIndex: 0, Desc: false}},
	)
	first2 := planner.NewLimit(sortOp, 2, 0)
	rows, err := planner.Collect(first2)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println("First 2 users by age ASC (NULL first), then id:")
	for _, r := range rows {
		fmt.Printf("  id=%s name=%s age=%s\n",
			r.Values[0].String(), r.Values[1].String(), r.Values[2].String())
	}

	// Inner hash join users.id = orders.user_id.
	join := planner.NewHashJoin(
		planner.NewSeqScan(users, nil),
		planner.NewSeqScan(orders, nil),
		0, 1, planner.InnerJoin,
	)
	jrows, err := planner.Collect(join)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println("Inner join users x orders:")
	for _, r := range jrows {
		fmt.Printf("  name=%s order_id=%s amount=%s\n",
			r.Values[1].String(), r.Values[3].String(), r.Values[5].String())
	}

	// Inspect the physical plan.
	plan := planner.NewHashJoin(
		planner.NewSeqScan(users, nil),
		planner.NewSeqScan(orders, nil),
		0, 1, planner.InnerJoin,
	)
	fmt.Println("Plan:")
	fmt.Print(planner.Explain(plan, 0))
}
```
...
```
First 2 users by age ASC (NULL first), then id:
  id=4 name=dave age=NULL
  id=2 name=bob age=25
Inner join users x orders:
  name=alice order_id=10 amount=99.9
  name=alice order_id=11 amount=49.5
  name=bob order_id=12 amount=200
  name=carol order_id=13 amount=5
Plan:
HashJoin[0]
  SeqScan[users] pred=(none)
  SeqScan[orders] pred=(none)
```

### Tests

The tests pin sort ordering (including NULL-first), limit, and the four join behaviors that matter most: an inner hash join's match count, a nested-loop LEFT join's NULL-padded unmatched row, a hash join that must return zero rows because both sides carry NULL keys, and a hash LEFT join that emits its unmatched build rows NULL-padded on the right.
...
The sort is correct when it is stable and NULL-first: a multi-key ORDER BY keeps equal-key rows in input order, and a NULL key sorts ahead of every concrete value, which is what `TestSortAndLimit` checks by expecting `dave` first. The limit must be a pure streaming gate that stops the pipeline once it has emitted its quota. The joins are where the subtle correctness lives: an inner hash join emits only matches; a LEFT join preserves every left row, NULL-padding the ones that found no partner; and NULL keys never match, so two tables that both carry a NULL key still produce zero inner-join rows. If `TestHashJoinNullKeysNeverMatch` ever returns a row, the build phase is inserting NULL keys or the probe is looking them up — both are the sentinel bug SQL forbids. Confirm the whole suite is clean under `go test -race ./...`, and that `Explain` renders the plan you expect rather than `<unknown>`.
...
- [CMU 15-445 Query Execution](https://15445.courses.cs.cmu.edu/fall2024/slides/) — physical operators, nested-loop and hash join algorithms in depth.
- [pkg.go.dev/hash/fnv](https://pkg.go.dev/hash/fnv) — FNV-1a, used to hash join keys in the build phase.
- [pkg.go.dev/sort](https://pkg.go.dev/sort) — `sort.SliceStable` and its stability guarantee, relied on by the sort operator.

---

Back to [04-expression-evaluator-and-filter.md](04-expression-evaluator-and-filter.md) | Next: [06-group-by-and-aggregation.md](06-group-by-and-aggregation.md)
